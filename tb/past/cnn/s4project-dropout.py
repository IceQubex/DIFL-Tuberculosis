...
for d in range(len(datasets_directory)):
    ...
    x = layers.Conv2D(32,(3,3), padding="same", kernel_regularizer=regularizers.l2(0.01))(x)
    ...
    for epoch in range(epochs[d]):
        ...
        for i, (xbatch, ybatch) in enumerate(train_dataset):
            ...
            #print(f"Batch number: {i+1}/{len(train_dataset)}")
        ...
        if d==2 and float(test_accuracy.result()) > 0.7:
            break

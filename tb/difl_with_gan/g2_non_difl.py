...
secondary_img_height, secondary_img_width = 28,28 
num_of_filters = 256
trials = 5
overall_results = []
breaks = [20,40,60,80,100,120,140,160,180,200,220,240,260,280]
...
for t in range(trials):
    ...
    generator_model = keras.Model(inputs=inputs, outputs=outputs, name=f"DIFL_Generator_Model_{t}")
    ...
    # define the final model
    discriminator_model = keras.Model(inputs=inputs1, outputs=outputs1, name=f"DIFL_Discriminator_Model_{t}")
    ...
    # define the final model
    classification_model = keras.Model(inputs=inputs2, outputs=outputs2, name = f"Classification_Model_{t}")
    ...
    while epoch < epochs:
        ...
        res += str(float(classification_accuracy.result())) + ","
        classification_accuracy.reset_states()
    ...
    if option == "N" or option == "n":
        break
    '''
    overall_results.append(temp)

print("Results!\n")

for i in range(len(overall_results[0])):
    for j in range(len(overall_results)):
        print(overall_results[j][i])
    print("")

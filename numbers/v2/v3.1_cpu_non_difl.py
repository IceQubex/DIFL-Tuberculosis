...
num_of_intermediate_filters = 64
...
domain1_train_directory = sys.argv[1]
domain1_test_directory = sys.argv[2]
domain2_directory = sys.argv[3]
epochs = int(sys.argv[4])

print(f"\nImporting the necessary datasets..")

# import the first dataset
domain1_train_dataset = keras.preprocessing.image_dataset_from_directory(domain1_train_directory, labels='inferred', label_mode='int', color_mode='grayscale', batch_size=16, image_size=(28,28), seed=1)
domain1_test_dataset = keras.preprocessing.image_dataset_from_directory(domain1_test_directory, labels='inferred', label_mode='int', color_mode='grayscale', batch_size=16, image_size=(28,28), seed=1)

# import the first dataset
domain2_train_dataset = keras.preprocessing.image_dataset_from_directory(domain2_directory, labels='inferred', label_mode='int', color_mode='grayscale', batch_size=16, image_size=(28,28), validation_split=0.2, subset='training', seed=99999)
domain2_test_dataset = keras.preprocessing.image_dataset_from_directory(domain2_directory, labels='inferred', label_mode='int', color_mode='grayscale', batch_size=16, image_size=(28,28), validation_split=0.2, subset='validation', seed=99999)
...
x = layers.Conv2D(32, (3,3), activation = layers.LeakyReLU(alpha=0.2))(x)
x = layers.MaxPooling2D()(x)
x = layers.Conv2D(64, (3,3), activation = layers.LeakyReLU(alpha=0.2))(x)
x = layers.MaxPooling2D()(x)
outputs = layers.Conv2D(num_of_intermediate_filters, (3,3), activation = layers.LeakyReLU(alpha=0.2))(x)
...
# define input layer
inputs1 = keras.Input(shape=(3,3,num_of_intermediate_filters))
...
# define input layer
inputs2 = keras.Input(shape = (3,3,num_of_intermediate_filters))
...
classification_accuracy = keras.metrics.SparseCategoricalAccuracy()
...
for epoch in range(epochs):
    ...
    for i in range(len(domain1_train_dataset)):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            classification_loss = SCCE_loss(ybatchclass, logits)
...
print("Number of layers in Generator: 2")
print("Number of Intermediate Channels: ", num_of_intermediate_filters)

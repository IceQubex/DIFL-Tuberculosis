...
# define input layer
inputs1 = keras.Input(shape = (5,5,64))

# apply convolution and pooling layers
x = layers.Conv2D(128, (3,3), activation = 'relu')(inputs1)
x = layers.Flatten()(x)
...
less_optimizer = keras.optimizers.SGD(lr=1e-5, momentum=0.9)
...
for _ in range(1):
    print(f"Pre-training Round {_+1}!")
    for i in range(len(source_train)):
        ...
        print(difl_loss)
        gradients = tape.gradient(difl_loss, difl_model.trainable_weights)
        less_optimizer.apply_gradients(zip(gradients, difl_model.trainable_weights))
...
for epoch in range(epochs):
    ...
    for i in range(len(source_train)):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            logits2 = difl_model(xtargetbatch, training=True)
            difl_loss = mse_loss(logits1, logits2)
            logits3 = classification_model(logits1, training=True)
            classification_loss = categorical_loss(ysourcebatch, logits3)
            loss = classification_loss + difl_loss
        gradients_difl = tape.gradient(loss, difl_model.trainable_weights)
        gradients_classification = tape.gradient(loss, classification_model.trainable_weights)
        
        del tape

        # apply the gradients
        optimizer.apply_gradients(zip(gradients_difl, difl_model.trainable_weights))
        optimizer.apply_gradients(zip(gradients_classification, classification_model.trainable_weights))
